...
class ClickModel:
    ...
    def test(self, sessions, reportPositionPerplexity=True):
        ...
        for s in sessions:
            ...
            # logLikelihood += math.log(sum(clickProbs[i][N - 1] * intentWeight[i] for i in possibleIntents)) / N # 等于最后一位处的点击概率，为啥要除N
            logLikelihood += math.log(sum(clickProbs[i][N - 1] * intentWeight[i] for i in possibleIntents))
            ...
            for k, click in enumerate(s.clicks):
                ...
                logProb = math.log(sum(curClick[i] * intentWeight[i] for i in possibleIntents), 2) - math.log(
                    sum(prevClick[i] * intentWeight[i] for i in possibleIntents), 2) # 为啥要减去上一位的？因为这里需要用条件概率嘛，但直接除得到的是EPi,Vi，而不是EPi
    ...
    def test_one_by_one(self, sessions):
        """
            Evaluates the prediciton power of the click model for a given sessions.
            Returns the log-likelihood, perplexity, position perplexity
            (perplexity for each rank a.k.a. position in a SERP)
        ...
        ll_position = []
        for i in range(N):
            ll_position.append(math.log(clickProbs[False][i], 2))

        return logLikelihood / len(sessions), ll_position
