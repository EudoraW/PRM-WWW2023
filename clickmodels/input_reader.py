...
import json
from .config_sample import VPT_EPSILON
        ...
        for line in f:
            ...
            if self.read_viewport_time:
                ...
                if self.discard_no_viewport and sum(vpt) <= 0:
                    continue
                for rank, (c, v1, v2) in enumerate(zip(clicks, vpt, w_vpt)):
                    if c == 1:
                        vpt[rank] = max(VPT_EPSILON, v1)
                        w_vpt[rank] = max(VPT_EPSILON, v2)
            urlsObserved = 0
